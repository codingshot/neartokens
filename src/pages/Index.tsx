...
import { Calendar, TrendingUp, DollarSign, Users, Coins, Clock, ArrowRight } from 'lucide-react';
...
const Index = () => {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#f2f1e9] flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 font-medium">Error loading token data</p>
        </div>
      </div>
    );
  }

  // Combine token sales and listings for projects
  const allProjects = [
    ...(tokenData?.token_sales || []).map((project: any) => ({ ...project, type: 'sale' })),
    ...(tokenData?.token_listings || []).map((project: any) => ({ ...project, type: 'listing' }))
  ];
  ...
  const stats = [
    {
      ...
      value: tokenData?.statistics?.total_projects || 0,
      icon: Coins,
      color: "text-[#00ec97]"
    },
    {
      title: "Token Sales",
      value: tokenData?.statistics?.total_token_sales || 0,
      icon: TrendingUp,
      color: "text-[#ff7966]"
    },
    {
      title: "Token Listings",
      value: tokenData?.statistics?.total_token_listings || 0,
      icon: Clock,
      color: "text-[#17d9d4]"
    },
    {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-black/60 font-medium">Tokens on NEAR, launch schedule</p>
            </div>
